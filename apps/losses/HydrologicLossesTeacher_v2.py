...
FINAL_TARGET_PCT = 74.0
FINAL_TARGET_TIME_HR = 24.0
...
def first_crossing_time(time_hr: np.ndarray, values_pct: np.ndarray, target_pct: float) -> Optional[float]:
    """Estimate the first crossing time using linear interpolation within a time step."""
    times = np.concatenate(([0.0], np.asarray(time_hr, dtype=float)))
    values = np.concatenate(([0.0], np.asarray(values_pct, dtype=float)))
    idx = np.where(values >= target_pct)[0]
    if len(idx) == 0:
        return None
    ...
    lower = upper - 1
    y0 = float(values[lower])
    y1 = float(values[upper])
    if abs(y1 - y0) <= 1e-12:
        return float(times[upper])

    fraction = (target_pct - y0) / (y1 - y0)
...
def make_goal_table(result: LossResult) -> pd.DataFrame:
    ...
    rows = []
    for target_pct, target_time_hr in CHECKPOINT_TARGETS:
        crossing = first_crossing_time(result.time_hr, cumulative_runoff_pct, target_pct)
        if crossing is None:
            current_time = "Not reached"
            miss = "--"
        else:
            current_time = f"{crossing:.2f} hr"
            miss = f"{crossing - target_time_hr:+.2f} hr"

        rows.append(
            {
                "Target": f"{target_pct:.0f}% runoff",
                "Target Time": f"{target_time_hr:.0f} hr",
    ...
    rows.append(
        {
            "Target": f"{FINAL_TARGET_PCT:.0f}% final runoff",
            "Target Time": f"{FINAL_TARGET_TIME_HR:.0f} hr",
            "Current Time": f"{FINAL_TARGET_TIME_HR:.0f} hr",
            "Miss": f"{final_runoff_pct - FINAL_TARGET_PCT:+.2f} %-pt",
        }
    )
...
def make_cumulative_figure(result: LossResult) -> go.Figure:
    ...
    target_points = CHECKPOINT_TARGETS + [(FINAL_TARGET_PCT, FINAL_TARGET_TIME_HR)]
    fig.add_trace(
        go.Scatter(
            x=[target_time for target_pct, target_time in target_points],
            y=[target_pct for target_pct, target_time in target_points],
            mode="markers+text",
            name="Runoff targets",
            text=[f"{target_pct:.0f}%" for target_pct, target_time in target_points],
            textposition="top center",
            marker={"color": FNI_ORANGE, "size": 11, "symbol": "x", "line": {"width": 2}},
            hovertemplate="Target: %{y:.0f}% runoff<br>Time: %{x:.0f} hr<extra></extra>",
        )
    )

    fig.update_layout(
        title="Cumulative Rainfall, Runoff, Losses, and Targets",
        xaxis_title="Time (hr)",
                ...
                html.Div(
                    ...
                    children=[
                        ...
                        html.Div(
                            ...
                            children=[
                                # Primary working area: cumulative response and compact goal tracker
                                html.Div(
                                    style={
                                        "display": "grid",
                                    ...
                                    children=[
                                        html.Div(
                                            ...
                                            style={**CARD_STYLE, "minHeight": "492px", "minWidth": 0},
                                        ),
                                        html.Div(
                                            style={**CARD_STYLE, "minHeight": "492px", "minWidth": 0},
                                            children=[
                                                ...
                                                html.P(
                                                    "Checkpoint miss is measured in hours. Final-runoff miss is measured in percentage points.",
                                                    style={"fontSize": "13px", "color": FNI_DARK_GRAY, "marginBottom": "12px"},
                                                ),
                                                dash_table.DataTable(
                                                    id="goal-table",
                                                    columns=[
                                                        {"name": "Target", "id": "Target"},
                                                        {"name": "Target Time", "id": "Target Time"},
                                                    ...
                                                    style_data_conditional=[
                                                        ...
                                                        {"if": {"row_index": 5}, "backgroundColor": "#f7faee", "fontWeight": "bold"},
                                                    ],
                                                ),
                                            ],
                                        ),
                                ...
                                # Larger full-width incremental teaching plot
                                html.Div(
                                    dcc.Graph(
                                        id="incremental-plot",
                                        config={"displayModeBar": False, "responsive": True},
                                ...
                                # Supporting metrics follow the two primary teaching outputs
                                html.Div(
                                    style={
                                        "display": "grid",
